...
class Like
{
    ...
    )
    {
        ...
        $stmt = $this->db->prepare('SELECT uid,gid FROM posts where id = :pid');
        $stmt->bindParam(':pid', $pid);
        $stmt->execute();
        $flagUserExists = false;
        while ($row = $stmt->fetch()) {
            if (!(new User())->setId(intval($row['uid']))->isFriendWith($uid) && intval($row['uid']) !== $uid && ($row['gid'] && !(new Group())->setId(intval($row['gid']))->hasMember($uid))) {
